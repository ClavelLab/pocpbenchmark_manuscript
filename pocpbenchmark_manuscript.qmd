...
We quantify our findings on the confusion matrix using the  Matthews Correlation Coefficient [MCC, @chiccoAdvantagesMatthewsCorrelation2020]
...
But diving into the results, and separating the bacterial families indicate a different story.

```{r}
# mcc description
tar_read(all_pocpu) %>% count(Family,class) %>% 
filter(Family == "f__Lactobacillaceae") %>% arrange(desc(class)) %>% mutate(n = prettyNum(n,big.mark = " ")) %>% glue::glue_data("{class} = {n}") %>% glue::glue_collapse(sep = ", ", last = " and ")
```
