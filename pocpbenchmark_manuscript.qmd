...
In the last three years only, the number of bacterial genomes available has has by 35 000 yearly additions to the RefSeq collection, both from isolates and metagenomes [@haftRefSeqProkaryoticGenome2024]. To analyse these genomes, we need clear and quick methods to taxonomically assign genomes. 
...
In @qinProposedGenusBoundary2014, guidance was provided on how to implement the computation of POCP, including the use of BLASTP, as implemented in Protologger [@hitch_automated_2021].
...
#| fig-dpi: 300
plot_grid(
  tar_read(fig_delta_proteome_pocpu),
  tar_read(fig_delta_genome_pocpu), nrow = 2, 
  labels = "AUTO", axis = "l"
)
```
