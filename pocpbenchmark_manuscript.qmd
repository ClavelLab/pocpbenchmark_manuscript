...
Fold change of computing metrics for the 10 approaches used in the benchmark compared to the BLAST_BLASTP approach.
...
BLASTPDB, the database approach to BLASTP (@tbl-tools-parameters) that allows for parallel computations, was only half the time of BLASTP on average, but DIAMOND and MMSEQS2 based approaches were respectively ran in a `r time_fraction_category[["DIAMOND"]]` and `r time_fraction_category[["MMSEQS2"]]` of the time of BLASTP.
...
Adequacy between POCP values computed with the legacy BLAST_BLASTP against the BLAST_BLASTPDB approach that build databases before alignment.
...
BLASTPDB produced exactly similar POCP values to BLASTP (@suppfig-pocp-blastdb). The other approaches do not perform as well.
All flavors of DIAMOND have a coefficient of determination ($R^2$) above 0.99 when matching their POCP values with the original from BLASTP, with the exception of the DIAMOND_FAST approach that deviates more from the expected values (@fig-blast-vs-all-pocp).
...
Adequacy between POCPu values computed with the legacy BLAST_BLASTP against the BLAST_BLASTPDB approach that build databases before alignment.
...
These higher-than-100% POCP values disappears when using the POCPu (@fig-blast-vs-all-pocpu and @suppfig-pocpu-blastdb).
...
The BLAST_BLASTPDB approach is a strong contender for a BLAST_BLASTP alternative because whilst using more resources, it performs exactly the same as BLAST_BLASTP (@suppfig-pocp-blastdb, @suppfig-pocpu-blastdb and @tbl-R2) in `r time_fraction_category[["BLAST"]]` of the time (@tbl-tools-metrics).
...
A more sustainable alternative would be DIAMOND_VERYSENSITIVE that performs respectively 10x and faster than BLAST_BLASTPDB,  BLAST_BLASTP, while still maintaining reasonable usage of the resources (@tbl-tools-metrics).
...
We use all the `r n_genomes` genomes and calculate POCP and POCPu for `r pairs_for_recommended` pairwise comparisons with the DIAMOND_VERYSENSITIVE approach (@fig-pocp-pocpu-densities).
...
With this procedure, the genus delineation of `r n_tentative[["improved"]]` families (`r improved_families`) was improved, with at least 0.1 point increase in MCC, and for `r n_tentative[["rescued"]]` additional families (`r rescued_families`) it was even rescued with a maximum MCC above 0.7.
...
Interestingly, in only `r n_category[["decrease"]]` cases, an optimal POCPu threshold had to be lowered (`r decrease_family`).
