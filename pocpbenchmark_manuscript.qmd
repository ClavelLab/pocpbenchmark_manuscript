...
We quantify our findings on the confusion matrix using the  Matthews Correlation Coefficient [MCC, @eq-mcc].
...
```{r confusion_matrix_family}
lactobacillaceae <- get_family_confusion_matrix(
  tar_read(pocpu_confusion_by_family), "f__Lactobacillaceae"
)
streptomycetaceae <- get_family_confusion_matrix(
  tar_read(pocpu_confusion_by_family), "f__Streptomycetaceae"
)
```
...
The large family of _Streptomycetaceae_ (Actinomycetota), for instance, exhibits `r streptomycetaceae`.
Not all is lost, as `r `tally_mcc_thresholds["n_70"]` families out of 35 show that POCPu delineates bacterial genera accurately (MCC $\geq$ 0.7). For instance, for all the comparisons done within the _Lactobacillaceae_ family (Bacillota), we see `r lactobacillaceae`.
