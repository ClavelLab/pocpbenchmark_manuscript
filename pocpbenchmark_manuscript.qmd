---
title: Fast and robust estimate of genus novelty with POCPu
affiliations:
  - id: 1
...
abstract: |
    Many biomes are extensively studied using sequencing techniques. Accurate taxonomic assignment of the microbial genomes and functions under study, obtained from isolates or metagenomes, is essential to identify novel taxa and to enable robust comparison between studies. However, the taxonomic placement of bacterial taxa above the species level is hampered by the lack of reliable quantitative measures to make informed decisions. In this work, we focused on bacterial classification at the genus level at a large scale using 4,767 genomes across 35 families and different isolation sources.

    We tested 10 protein alignment methods to calculate the Percentage Of Conserved Proteins (POCP) and found evidence for a scalable yet robust alternative to BLASTP. However, due to limitations of the method, we introduce POCPu that showed better genus delineation than with POCP thanks to using only unique matches. This proposed method is 20x faster and more robust owing to using very-sensitive settings of DIAMOND, whilst also enhancing sustainability. In summary, POCPu improves bacterial genus assignment by facilitating comparative analysis of an increasingly larger number of genomes.
...
Two elements show that accurate taxonomic placement of microorganisms is more important now than ever: (i) a very high fraction of bacteria, both in the environment and host-associated microbiomes, remain to be described and named [@sutcliffeAddressingSublimeScale2021; @thomasMultipleLevelsUnknown2019;@dutkiewiczProposalPatescibacteriumDanicum2025]; (ii) large-scale metagenomic studies in the last decade and now high-throughput cultivation methods are accelerating the pace of bacterial discovery [@hugenholtzProkaryoticTaxonomyNomenclature2021].
It is therefore essential to consolidate the system for classifying bacteria and confidently assess their taxonomic novelty. It is equally important to ensure that the boundaries between genera that include known taxa are stable.
Whilst bacteria had been classified based on morphology and phenotypic parameters for decades, the advent of genomics has revolutionised the way we classify bacteria, giving rise to a plethora of Overall Genome Relatedness Indices [@chunIntegratingGenomicsTaxonomy2014].
...
Furthermore, given the increased number of genomic resources available, we need a fast and reproducible framework to regularly assess taxonomic novelty as part of investigating microbial ecosystems functioning.
...
The system works best when resources and tools follow FAIR principles [@NationalMicrobiomeData; @NFDI4Microbiota; @wilkinsonFAIRGuidingPrinciples2016a], as done in this work.
Plus, we underlined that POPCu improves sustainability through faster computations and hence reduced electrical consumption, although the additional benefit of decreased disk usage is not captured by the carbon footprint estimator [@lannelongueGreenAlgorithmsQuantifying2021].
...
While our analysis included phylogenetically diverse taxa sourced from various environments, it was limited to the taxa within GTDB [@parksCompleteDomaintospeciesTaxonomy2020; @parksGTDBOngoingCensus2022] and focused on the dominant bacterial fraction. Despite the comprehensive nature of GTDB, our final genome selection might have missed important taxonomic groups of interest to readers. However, filtering the studied taxa was necessary to obtain enough data points per taxa to ensure statistical robustness.
Still, we kept enough genomes to represent more bacterial diversity than restricting our analyses to the type species of genera.
Previous studies on many-vs-many proteins alignment comparison used less phylogenetic diversity: 4 genomes from 4 genera in @hernandez-salmeronProgressQuicklyFinding2020, maximum 167 genomes from 5 genera in @holzerPOCPnfAutomaticNextflow2024.
Riesco et al. @riescoUpdateProposedMinimal2024 evaluated much more genomes -- 1,573 type strains  of type species of genera -- but calculated POCP -- using Bio-Py [@linSilentGeneBiopyBiopy2021] -- only to be compared with AAI and not to evaluate genus delineation.
...
However, one should only deviate from the standard threshold of 50% if the benefit is greater than the risk of creating more confusion. We have provided tentative thresholds for several families, for which confidence was high.
Indeed, initiatives like the SeqCode [@hedlundSeqCodeNomenclaturalCode2022; @jimenezSeqCodeGoldenAge2024] require reliable methods to assess taxonomic novelty, and we propose that POCPu is a robust yet scalable approach for modern taxonomy.
Finally, it is important to remember that accurate taxonomic placement is best achieved when multiple lines of evidence are considered, as implemented in Protologger [@hitchAutomatedAnalysisGenomic2021].
