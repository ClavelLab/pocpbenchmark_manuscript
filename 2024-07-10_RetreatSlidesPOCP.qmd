...
p_pocp <- targets::tar_read(all_pocp) %>% 
  ...
  scale_fill_okabe_ito(labels = targets::tar_read(pocp_group_sizes))+
  geom_vline(xintercept = 50, linetype="dashed")+
  # theme(legend.position = "inside", legend.position.inside = c(0.65, 0.75))+
  labs(x="POCP with\nDIAMOND very-sensitive", fill = "Gold\nstandard", y = "Density")+
...
p_ridges <- targets::tar_read(pocpu_ridges) %>% 
  ...
  geom_density_ridges(alpha=0.5, fill =  "#56B4E9")+
  geom_vline(xintercept = 50, linetype="dashed")+
  theme_cowplot(rel_small = 10/14)+
  labs(x="Per-Family POCPu (Intra-genera)",
       fill = "",y = "Family (n genomes included)")+
